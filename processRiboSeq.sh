...
# workflow for processing HUMAN ribosome profiling:
...
# 8. Run Ribotish for QC
# """

if [ $# -eq 0 ]
then
	echo "Pipeline to process Ribosome Profiling datasets, starting from fastq (NOT fastq.gz). Please modify the fastq file name first to get rid of unwanted parts of the name."
	echo "This is for ribosome profiling datasets that are demultiplexed and with adapter trimmed."
	echo "Usage: processRiboSeq.sh [options] Riboseq.fastq [Riboseq.fastq]" 
	echo ""
	echo -e "\tOptions:"
	echo -e "\t-----------------"
	...
	echo -e "\t-s)\tPath to .positions file for cs count (see Plastid)"
	echo -e "\t-p)\tNumber of processes to use."
	echo -e "\t-m)\tMax length of footprint to consider (for make_wiggle and cs count)"
	echo -e "\t-n)\tMin length of footprint to consider (for make_wiggle and cs count)"
	echo -e "\t-o)\tOffset for P-site alignment (for make_wiggle and cs count)"
	...
	do
		...
		# # 	echo $fq
		# 	zcat $fqgz | fastx_clipper -a $adapter_sequence -ncv -Q33 -o $fq
		...
		make_wiggle --count_files alignments/${fstem}.bam  --min_length $minLength --max_length $maxLength --fiveprime --offset $offset -o igv/$fstem > /dev/null
    	...
    	cs count --count_files alignments/${fstem}.bam --min_length $minLength --max_length $maxLength --fiveprime --offset $offset $cs_count_positions_file counts/${fstem}_counts > /dev/null


    	echo "Running ribotish quality"
    	# Use ribotish package to check quality of dataset
    	ribotish quality -b alignments/${fstem}.bam -g $gtf_file -l 25,40 -p $nproc
